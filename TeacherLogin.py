...
import TeacherLogin_support

def vp_start_gui():
    '''Starting point when module is the main routine.'''
    global val, w, root
    root = tk.Tk()
    top = TeacherLogin (root)
    TeacherLogin_support.init(root, top)
    root.mainloop()

w = None
def create_TeacherLogin(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = TeacherLogin (w)
    TeacherLogin_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_TeacherLogin():
    global w
    w.destroy()
    w = None
...
class TeacherLogin:
    def __init__(self, top=None):
        ...
        self.Login.configure(command=self.TeacherLogin)
        ...
        self.Label3.configure(text='''Login As Teacher''')

        self.menubar = tk.Menu(top,font="TkMenuFont",bg=_bgcolor,fg=_fgcolor)
        top.configure(menu = self.menubar)

    def TeacherLogin(self):
        TeacherLogin_support.TeacherLogin(self.Email.get(),self.Password.get())

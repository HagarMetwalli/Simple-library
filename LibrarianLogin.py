...
import LibrarianLogin_support

def vp_start_gui():
    '''Starting point when module is the main routine.'''
    global val, w, root
    root = tk.Tk()
    top = LibrarianLogin (root)
    LibrarianLogin_support.init(root, top)
    root.mainloop()

w = None
def create_LibrarianLogin(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = LibrarianLogin (w)
    LibrarianLogin_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_LibrarianLogin():
    global w
    w.destroy()
    w = None
...
class LibrarianLogin:
    def __init__(self, top=None):
        ...
        self.Login.configure(command=self.LibrarianLogin)
        ...
        self.Label3.configure(text='''Login As Librarian''')

        self.menubar = tk.Menu(top,font="TkMenuFont",bg=_bgcolor,fg=_fgcolor)
        top.configure(menu = self.menubar)

    def LibrarianLogin(self):
        LibrarianLogin_support.LibrarianLogin(self.Email.get(),self.Password.get())

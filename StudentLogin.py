...
import StudentLogin_support

def vp_start_gui():
    '''Starting point when module is the main routine.'''
    global val, w, root
    root = tk.Tk()
    top = StudentLogin (root)
    StudentLogin_support.init(root, top)
    root.mainloop()

w = None
def create_StudentLogin(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = StudentLogin (w)
    StudentLogin_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_StudentLogin():
    global w
    w.destroy()
    w = None
...
class StudentLogin:
    def __init__(self, top=None):
        ...
        self.Login.configure(command=self.StudentLogin)
        ...
        self.Label3.configure(text='''Login As Student''')

        self.menubar = tk.Menu(top,font="TkMenuFont",bg=_bgcolor,fg=_fgcolor)
        top.configure(menu = self.menubar)

    def StudentLogin(self):
        StudentLogin_support.StudentLogin(self.Email.get(),self.Password.get())
